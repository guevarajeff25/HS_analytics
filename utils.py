...
# ------------------------------------------------------------
# File helpers (Streamlit uploads need seek(0) before re-reads)
# ------------------------------------------------------------
def _maybe_seek0(path_or_file):
    try:
...
# ------------------------------------------------------------
# Baseball IP helpers (supports 12.1 / 12.2 notation)
# ------------------------------------------------------------
...
def load_dataset(mode: str, batting_file=None, pitching_file=None, fielding_file=None):
    ...
    """
    if mode == "demo":
        bat = read_gamechanger_csv(os.path.join(DEMO_DIR, "batting.csv"))
        pit = read_gamechanger_csv(os.path.join(DEMO_DIR, "pitching.csv"))
        fld = read_gamechanger_csv(os.path.join(DEMO_DIR, "fielding.csv"))
    else:
    ...
    pit_aliases = {
        ...
        "H": ["H", "H_1", "HITS"],
        "R": ["R", "R_1", "RUNS"],
        "ER": ["ER", "ER_1", "EARNEDRUNS"],
        "BB": ["BB", "BB_1", "WALKS"],
        "SO": ["SO", "SO_1", "K", "STRIKEOUTS"],
        "HBP": ["HBP"],
        "HR": ["HR", "HR_1", "HOMERUNS"],
        "WP": ["WP", "WILDPITCHES"],
        "ERA": ["ERA"],
